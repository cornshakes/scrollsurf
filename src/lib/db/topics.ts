...
let stmts: {
  get_datasets: StatementSync;
  get_picture_dataset: StatementSync;
  get_picture_topics: StatementSync;
  get_top_levels: StatementSync;
  get_categories: StatementSync;
  get_topics: StatementSync;
} | null = null;
...
const s = () => {
  ...
  stmts = {
    get_datasets: db.prepare(`
      ...
      ORDER BY d.name
    `),
    get_picture_dataset: db.prepare(`
      SELECT
        d.name AS dataset,
        d.source_url,
...
export const get_category_tree = (user_id: number | null): CategoryTree => {
  const top_level_rows = s().get_top_levels.all({
    $user_id: user_id,
  }) as unknown as CategoryGroup[];
  const category_rows = s().get_categories.all({ $user_id: user_id }) as unknown as (TopicStat & {
    top_level: string;
  })[];
...
export const get_topic_tree = (user_id: number | null): TopicTree => {
  const article_dataset_rows = s().get_datasets.all({
    $user_id: user_id,
  }) as unknown as DatasetGroup[];
  const topic_rows = s().get_topics.all({ $user_id: user_id }) as unknown as (TopicStat & {
    dataset: string;
  })[];

  const pic_topic_rows = s().get_picture_topics.all({
    $user_id: user_id,
  ...
  for (const name of pic_datasets) {
    const row = s().get_picture_dataset.get({ $user_id: user_id, $dataset: name }) as
      | DatasetGroup
      | undefined;
    if (row) {
      article_groups.push({
