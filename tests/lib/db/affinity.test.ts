...
it('liked topic is over-represented in fetch (statistical)', () => {
  ...
  for (let i = 0; i < 200; i++) {
    ...
    y_ids.push(insert_article({ url: `https://a.t1.y/${i}`, topics: [TOPIC_Y] }));
  }
  for (let i = 0; i < 20; i++) {
    set_like('article', x_ids[i], uid, 1);
  }
  const result = get_next_articles_internal(100, uid);
...
it('disliked topic is under-represented and never fully excluded (statistical + deterministic)', () => {
  ...
  for (let i = 0; i < 20; i++) {
    set_like('article', y_ids[i], uid, -1);
  }
  const result = get_next_articles_internal(100, uid);
  ...
  for (const id of disliked) {
    set_like('article', id, uid2, -1);
  }
  const all = get_next_articles_internal(5000, uid2);
...
it('clicks alone boost a topic (statistical)', () => {
  ...
  for (let i = 0; i < 20; i++) {
    set_like('article', x_ids[i], uid, 0);
...
it('neutral user with zero signals gets all articles (deterministic)', () => {
  ...
  const result = get_next_articles_internal(50, uid);
...
it('anonymous user gets all articles and nothing is marked seen', () => {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    if (i < 5) {
      set_like('article', id, other, 1);
    }
  }
  const r1 = get_next_articles_internal(20, null);
  expect(r1).toHaveLength(10);
  const r2 = get_next_articles_internal(20, null);
...
it('likes from user A do not skew user B feed (statistical)', () => {
  ...
  for (let i = 0; i < 20; i++) {
    set_like('article', x_ids[i], uid_a, 1);
  }
  const result = get_next_articles_internal(100, uid_b);
  const x_count = result.filter((a) => a.topics.some((t) => t.topic === 'X')).length;
...
it('AFFINITY_STRENGTH=0 produces near-uniform results despite likes (statistical + deterministic)', () => {
  const x_ids: number[] = [];
  for (let i = 0; i < 200; i++) {
  ...
  const all = get_next_articles_internal(5000, uid_a, 0);
  expect(all).toHaveLength(380);

  // statistical: near-50/50 X/Y despite 20 liked X articles
  const uid_b = insert_user();
  for (let i = 0; i < 20; i++) {
    set_like('article', x_ids[i], uid_b, 1);
  }
  const result = get_next_articles_internal(100, uid_b, 0);
  const x_count = result.filter((a) => a.topics.some((t) => t.topic === 'X')).length;
  expect(x_count).toBeGreaterThan(35);
  expect(x_count).toBeLessThan(65);
});
...
it('pictures: liked topic is over-represented in fetch (statistical)', () => {
  ...
  const result = get_next_pictures_internal(100, uid);
