...
it('batch fetch: each picture gets exactly its own links with no cross-leak', () => {
  ...
  const p1_topics = p1.links
    .filter((l) => l.type === 'topic')
    .map((l) => l.title)
    .sort();
  expect(p1_topics).toEqual(['T1', 'T2']);
  const p2_topics = p2.links.filter((l) => l.type === 'topic').map((l) => l.title);
  expect(p2_topics).toEqual(['T3']);
});
