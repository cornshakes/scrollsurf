// Builds the committable seed DB at `e2e/fixtures/scrollsurf-e2e-test.db` from
// local `datasets/*.db` reference files. Run once by anyone who has all datasets:
//   npm run test:e2e:create-db
// Commit the resulting `e2e/fixtures/scrollsurf-e2e-test.db` so others can run
// e2e tests without downloading datasets.
